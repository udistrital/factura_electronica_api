...
from Scriptdb.servicios.notaDebito import * 
...
def emitDebitNote(req=""):
    try:
        ...
        code = consultaCodigoNotaDeb(conexion,'')
        ...
        if not exec_flag or estado_envio == "C":
            ...
            resultado = {
                ...
                "resultado": {
                    "estado": "error",
                    "mensaje": "No fue posible procesar la emisión de la Nota Debito a Titanio",
                    "datos": {
                        "detalle": "Ya existe una solicitud de emisión de la Nota Debito a Titanio, con errores que se deben corregir"
                    }
                }
            }
            respuesta = loadData(resultado, conexion)
            ...
            resultado = {
                ...
                "resultado": {
                    ...
                    "mensaje": "No fue posible procesar la emisión de la Nota Debito a Titanio",
                    "datos": {
                        "detalle": f"Ya existe una emisión vigente de la Nota Debito, con el número de transacción {tr_id}"
                    }
                }
            }
            respuesta = loadData(resultado, conexion)
        ...
        respuesta = {"Error":"No fue posible procesar la emisión de la Nota Debito a Titanio"}
        return respuesta
    #finally:
    #    conexion.close()        
...
def extractData(busqueda="",conexion=""):
    try:
        ...
        data = consultaRegistrosNotaDeb(conexion,recibo)
...
def transformData(req,resultado):
    try:
        ...
        if resultado :     
            ...
            if not acceso.get("succes") or not acceso.get("token"):
                ...
                try:
                    ...
                    if factura.get("error_id") > 0:
                        ...
                        respuesta['code'] = 422
                        respuesta['resultado']['estado'] = "error"
                        respuesta['resultado']['mensaje'] = "No fue posible procesar la solicitud de emitir la Nota Debito a Titanio."
                        ...
                        respuesta['resultado']['mensaje'] = (
                            f"Solicitud de generación de Nota Debito electrónica en Titanio, con estado {estado_dian}."
                        )
                        respuesta['resultado']['datos'] = datos_transaccion
                        return respuesta
                    ...
                    respuesta["code"] = 504
                    respuesta["resultado"]["estado"] = "error"
                    respuesta["resultado"]["mensaje"] = "Timeout al emitir la Nota Debito en Titanio."
                    ...
                    respuesta["code"] = 502
                    respuesta["resultado"]["estado"] = "error"
                    respuesta["resultado"]["mensaje"] = "Error de comunicación con Titanio al emitir la Nota Debito."
                    respuesta["resultado"]["datos"] = {"detalle": str(e)}
                    return respuesta
                except Exception as e:
                    ...
                    respuesta['code'] = 422
                    respuesta['resultado']['estado'] = "error"
                    respuesta['resultado']['mensaje'] = "No fue posible la solicitud de emitir la Nota Debito a Titanio."   
                    return respuesta
        else:
            #print("Ocurrió un error al emitir la factura: ", e)
            respuesta['status'] = "error"
            respuesta['code'] = 404
            respuesta['resultado']['estado'] = "error"
            respuesta["resultado"]["mensaje"] = "Error al solicitar la emisión de la Nota Debito en Titanio."
